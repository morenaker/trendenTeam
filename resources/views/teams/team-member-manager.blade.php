<div>
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-form-section submit="addTeamMember">
                ...
                <x-slot name="description">
                    {{ __('Přidejte do svého týmu nového člena týmu.') }}
                </x-slot>

                <x-slot name="form">
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600">
                            {{ __('Zadejte e-mailovou adresu osoby, kterou chcete přidat do tohoto týmu.') }}
                        </div>
                    </div>
    ...
    @if ($team->teamInvitations->isNotEmpty() && Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('Nevyřízené pozvánky týmů') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Tito lidé byli pozváni do vašeho týmu a byl jim zaslán e-mail s pozvánkou. Mohou se připojit k týmu přijetím e-mailové pozvánky.') }}
                </x-slot>
    ...
    @if ($team->users->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                ...
                <x-slot name="description">
                    {{ __('Všichni tito lidé, kteří jsou součástí tohoto týmu.') }}
                </x-slot>
    ...
    <x-jet-confirmation-modal wire:model="confirmingLeavingTeam">
        ...
        <x-slot name="content">
            {{ __('Jste si jistý, že chcete opustit tento tým?') }}
        </x-slot>
    ...
    <x-jet-confirmation-modal wire:model="confirmingTeamMemberRemoval">
        <x-slot name="title">
            {{ __('Odebrat člena týmu') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Opravdu chcete tohoto člověka odebrat z týmu?') }}
        </x-slot>
